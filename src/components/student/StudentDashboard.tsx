...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  const [timeMode, setTimeMode] = useState<'real' | '10:15' | '11:00' | '11:45' | '12:30' | '01:15' | '02:00'>('real');
  ...
  const tzAbbr = React.useMemo(() => {
    try {
      ...
      return tzPart ? tzPart.value : userTimezone;
    } catch {
      return userTimezone;
    }
  }, [now, userTimezone]);
  ...
  const recentGrades = submissions.filter(s => s.studentId === currentUser.id && s.status === 'graded');

  const handleAiSubjectHelp = (subject: string) => {
    setAiTutorInitialPrompt(`Help me understand key concepts and practice questions for ${subject}.`);
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-[#FDEEDC] text-xs font-bold">
              <span>🇳🇵 Grade {studentData.gradeLevel} '{studentData.section}' • Roll No. {studentData.rollNumber}</span>
            </div>
            ...
            <p className="text-[#F9F7F2]/90 text-xs md:text-sm max-w-xl">
              Here is your clean daily schedule and learning tasks for Mount Everest Secondary School.
            </p>
          </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-3xl p-6 border border-[#EDEAE2] shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#EDEAE2] pb-4">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-[10px] font-black px-2.5 py-1 rounded-full bg-[#EBF1E8] text-[#4A6741] border border-[#88A070]/40 flex items-center gap-1.5 shadow-xs">
                    ...
                    <span>Clock: {now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })} ({tzAbbr})</span>
                  </span>
                </div>
                
                <p className="text-xs text-[#7A7A72] mt-0.5">
                  Timezone: <span className="font-semibold text-[#2D2D2A]">{userTimezone}</span>. Active period highlights automatically when your local clock matches period hours.
                </p>
              </div>

              {/* Time Mode Simulator / Selector */}
              <div className="flex items-center gap-2 self-start sm:self-auto">
                <div className="flex items-center gap-1.5 bg-[#F9F7F2] p-1.5 rounded-2xl border border-[#EDEAE2]">
                  ...
                  >
                    <option value="real">⏱️ Real Clock ({tzAbbr})</option>
            ...
            {selectedDay === systemTodayName && (
              <div className={`p-3 rounded-2xl border text-xs font-bold flex items-center justify-between gap-2 transition-all ${
                activePeriod
                  ? 'bg-[#EBF1E8] border-[#88A070] text-[#4A6741]'
                  : 'bg-[#F9F7F2] border-[#EDEAE2] text-[#7A7A72]'
              }`}>
                <div className="flex items-center gap-2">
                  <span className={`w-2.5 h-2.5 rounded-full ${activePeriod ? 'bg-[#4A6741] animate-ping' : 'bg-amber-500'}`}></span>
                  <span>
                    {activePeriod ? (
                      <>Live Class in Session: <strong className="text-[#2D2D2A]">Period {activePeriod.periodNumber} ({activePeriod.subject})</strong> • {activePeriod.startTime} - {activePeriod.endTime}</>
                    ) : effectiveMinutes < 600 ? (
                      <>Morning before school hours ({tzAbbr}). First period starts at <strong className="text-[#2D2D2A]">10:00 AM</strong>.</>
                    ) : effectiveMinutes >= 870 ? (
                      <>School concluded for today ({tzAbbr}). All 6 periods completed!</>
                    ) : (
                      <>School Recess / Transition Break ({tzAbbr}). Next period starts shortly.</>
                    )}
                  </span>
                </div>
            ...
            <div className="space-y-2.5 pt-1">
              {currentDayPeriods.length === 0 ? (
                <div className="text-center py-8 bg-[#F9F7F2] rounded-2xl border border-[#EDEAE2]">
                  ...
                  <p className="text-xs text-[#7A7A72] mt-0.5">No school periods scheduled for today. Enjoy your weekend!</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-2xl font-black text-xs flex flex-col items-center justify-center shrink-0 ${
                            highlighted
                              ? 'bg-[#4A6741] text-white shadow-sm'
                              : 'bg-[#E5E1D8] text-[#2D2D2A]'
                          }`}>
                            <span className="text-[9px] uppercase font-medium opacity-80">Period</span>
                            <span className="text-sm font-black leading-none">{period.periodNumber}</span>
                          </div>

                          <div>
                            <div className="flex items-center gap-2">
                              <h3 className="font-bold text-sm text-[#2D2D2A] font-serif">{period.subject}</h3>
                              {highlighted && (
                        ...
                        {period.requiredBooks && (
                          <div className={`shrink-0 border px-3 py-1.5 rounded-xl flex items-center gap-2 text-xs transition-colors ${
                            highlighted
                            ...
                            <div>
                              <span className="text-[10px] font-bold text-[#7A7A72] uppercase block leading-none">Pack in Bag:</span>
                              <span className="font-bold text-[#2D2D2A] text-[11px]">{period.requiredBooks}</span>
                            </div>
                          </div>
                        )}
                  ...
                  pendingAssignments.map((asg) => (
                    ...
                    >
                      ...
                      <div className="pt-2 flex items-center justify-between">
                        <span className="text-[10px] text-[#7A7A72] font-semibold">{asg.totalPoints} Marks</span>
                        <button
                ...
                {availableQuizzes.length === 0 ? (
                  ...
                  availableQuizzes.map((quiz) => (
                    ...
                    >
                      ...
                      <div className="pt-2 flex items-center justify-between">
                        <span className="text-[10px] text-[#7A7A72] font-semibold">{quiz.questions.length} Questions</span>
                        <button
