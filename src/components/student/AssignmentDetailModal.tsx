...
import { X, FileText, Upload, Sparkles, CheckCircle, Send, Paperclip, Download, AlertCircle } from 'lucide-react';
...
export const AssignmentDetailModal: React.FC<AssignmentDetailModalProps> = ({ assignmentId, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const finalResponseText = responseText.trim() || (isBelowClass9 ? `Submitted directly by Grade ${studentGrade} student.` : '');
      ...
      <div className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-[#EDEAE2] flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-5 natural-banner text-white flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[#F9F7F2]/90">{assignment.classroomName} • {assignment.totalPoints} Marks</p>
          </div>
        ...
        <div className="border-b border-[#EDEAE2] px-5 flex gap-6 text-xs font-bold bg-[#F9F7F2]">
          <button
            ...
            className={`py-3 border-b-2 transition-all ${
              activeTab === 'submit' ? 'border-[#4A6741] text-[#4A6741] font-extrabold' : 'border-transparent text-[#7A7A72]'
            }`}
          >
          ...
          <button
            ...
            className={`py-3 border-b-2 transition-all flex items-center gap-1.5 ${
              activeTab === 'ai-helper' ? 'border-[#E88D67] text-[#E88D67] font-extrabold' : 'border-transparent text-[#7A7A72]'
            }`}
          >
          ...
          {activeTab === 'submit' ? (
            <>
              ...
              <div className="p-4 rounded-2xl bg-[#F9F7F2] border border-[#EDEAE2] space-y-2">
                <h4 className="font-bold text-[#2D2D2A] text-xs font-serif">Teacher Instructions:</h4>
                <p className="text-[#2D2D2A] leading-relaxed font-sans">{assignment.instructions}</p>
                ...
                <div className={`p-4 rounded-2xl border space-y-2 ${
                  ...
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-[#4A6741] flex items-center gap-1.5 text-xs">
                      ...
                      Status: {existingSubmission.status === 'graded' ? `Graded (${existingSubmission.grade}/${assignment.totalPoints})` : 'Submitted & Awaiting Grading'}
                    </span>
                    <span className="text-[10px] text-[#7A7A72]">Submitted at {new Date(existingSubmission.submittedAt).toLocaleTimeString()}</span>
                  </div>

                  {existingSubmission.feedback && (
                    <div className="pt-2 border-t border-[#88A070]/30">
                      <p className="font-bold text-[#2D2D2A] text-[11px]">Teacher Feedback:</p>
                      <p className="text-[#2D2D2A] italic mt-0.5">"{existingSubmission.feedback}"</p>
                    </div>
                  )}
                </div>
              ) : null}
              ...
              <form onSubmit={handleSubmit} className="space-y-3 pt-2">
                ...
                {isBelowClass9 ? (
                  <div className="p-3.5 rounded-2xl bg-[#EBF1E8] border border-[#88A070]/40 flex items-start gap-2.5 text-[#2D2D2A]">
                    ...
                    <div>
                      <p className="font-bold text-xs text-[#4A6741]">Grade {studentGrade} Direct Submission Enabled</p>
                      <p className="text-[11px] text-[#7A7A72] mt-0.5">
                        Students below Class 9 do not need to upload files. Simply click <strong>{existingSubmission ? 'Resubmit' : 'Submit Homework'}</strong> below to turn in your assignment directly!
                      </p>
                    </div>
                  </div>
                ) : (
                  <div>
                    <label className="block font-semibold text-[#2D2D2A] mb-1">Attach Homework File (PDF/Image):</label>
                    <div className="flex gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => setFileName(`${currentUser.name.replace(/\s+/g, '_')}_Assignment.pdf`)}
                ...
                <div>
                  <label className="block font-semibold text-[#2D2D2A] mb-1">Notes / Typed Answers (Optional):</label>
                  <textarea
                    value={responseText}
                    onChange={(e) => setResponseText(e.target.value)}
                    rows={3}
                    placeholder={isBelowClass9 ? "Type any notes or answers here if requested by your teacher (optional)..." : "Write any additional notes, explanations or typed answers here..."}
                ...
                <div className="pt-2 flex justify-end gap-2">
                  ...
                  >
                    {isBelowClass9 ? <CheckCircle className="w-4 h-4" /> : <Upload className="w-4 h-4" />}
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-[#FDEEDC] border border-[#E88D67]/40 text-[#2D2D2A] space-y-1">
                ...
                <p className="text-[11px] opacity-90">Sikshya AI provides step-by-step conceptual hints without giving away final direct solutions, so you truly master the topic!</p>
              </div>
