...
export const ClassroomView: React.FC<ClassroomViewProps> = ({
  ...
  const handlePostSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newPostText.trim() || !currentClassroom) return;
  ...
  const filteredClassrooms = classrooms.filter(cls => {
    ...
    const matchesSubject = subjectFilter === 'all' || cls.subject.toLowerCase().includes(subjectFilter.toLowerCase());
      ...
      <div className="space-y-6 pb-16 animate-in fade-in duration-200">
        ...
        <div className="relative rounded-3xl overflow-hidden natural-banner text-white shadow-md p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-2 relative z-10">
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold font-serif">Enrolled Subjects & Classrooms</h1>
            <p className="text-xs md:text-sm text-[#F9F7F2]/90 max-w-2xl">
              Browse all your enrolled subject classrooms. Click on any subject below to access its stream announcements, textbook modules, downloadable notes, homework assignments, and teacher contacts.
            </p>
          </div>
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-3xl p-6 max-w-md w-full border border-[#EDEAE2] shadow-2xl space-y-4 animate-in zoom-in-95">
              ...
              <p className="text-xs text-[#7A7A72]">
                Ask your subject teacher for the class code, then enter it below (e.g., <strong className="text-[#2D2D2A]">MATH8A</strong>, <strong className="text-[#2D2D2A]">SCI8A</strong>, <strong className="text-[#2D2D2A]">NEP8A</strong>).
              </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredClassrooms.length === 0 ? (
            <div className="col-span-2 p-12 text-center bg-white rounded-3xl border border-[#EDEAE2] space-y-2">
              ...
              <p className="text-xs text-[#7A7A72]">Try clearing search or join a classroom with a class code.</p>
            </div>
          ) : (
            filteredClassrooms.map(cls => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`p-5 bg-gradient-to-r ${cls.colorTheme || 'from-emerald-700 to-teal-800'} text-white relative`}>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      ...
                      <div className="p-2.5 rounded-2xl bg-[#F9F7F2] border border-[#EDEAE2] flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="font-bold text-[#2D2D2A]">{clsAssignments.length} Assignments</p>
                        </div>
                      </div>
    ...
    <div className="space-y-6 pb-16 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-3xl border border-[#EDEAE2] shadow-xs">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-[#7A7A72] font-semibold hidden md:inline">Switch Subject:</span>
          <select
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono opacity-90 text-[#FDEEDC]">Code: {currentClassroom.code}</span>
            </div>
            <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight font-serif">{currentClassroom.name}</h1>
      ...
      <div className="border-b border-[#EDEAE2] flex gap-6 text-xs font-bold text-[#7A7A72]">
        {[
          ...
          { id: 'classwork', label: 'Classwork, Notes & Modules', icon: FileText },
      ...
      {activeTab === 'stream' && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white rounded-3xl p-4 border border-[#EDEAE2] shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-3">
              ...
              {classAssignments.length === 0 ? (
                ...
                classAssignments.map(asg => (
                  <div key={asg.id} className="text-xs space-y-1 pb-2 border-b border-[#EDEAE2] last:border-0">
                    <p className="font-semibold text-[#2D2D2A]">{asg.title}</p>
                    <p className="text-[10px] text-[#E88D67] font-bold">Due {asg.dueDate} at {asg.dueTime}</p>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            {/* Post Creation Box */}
            <form onSubmit={handlePostSubmit} className="bg-white rounded-3xl p-4 border border-[#EDEAE2] shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-3">
              ...
              <div className="flex justify-between items-center pt-1 border-t border-[#EDEAE2]">
                <div className="flex items-center gap-2 text-[#7A7A72]">
                  <button type="button" className="p-1.5 hover:bg-[#F9F7F2] rounded-lg text-xs flex items-center gap-1 text-[#7A7A72]">
            ...
            <div className="space-y-4">
              {classPosts.length === 0 ? (
                <div className="p-8 text-center bg-white rounded-3xl border border-[#EDEAE2] text-[#7A7A72] space-y-1">
                  ...
                  <p className="text-[11px]">Be the first to ask a question or start a study discussion!</p>
                </div>
              ) : (
                classPosts.map(post => (
                  <div key={post.id} className="bg-white rounded-3xl p-5 border border-[#EDEAE2] shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-[#7A7A72]">{new Date(post.createdAt).toLocaleString()}</p>
                        </div>
                      </div>
                      {post.pinned && (
                    ...
                    {post.attachments && post.attachments.length > 0 && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2">
                        {post.attachments.map(att => (
                          <div key={att.id} className="p-2.5 rounded-2xl bg-[#F9F7F2] border border-[#EDEAE2] flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2 truncate text-xs">
                              <FileText className="w-4 h-4 text-[#4A6741] shrink-0" />
                              <span className="font-semibold text-[#2D2D2A] truncate">{att.title}</span>
                            </div>
                            <Download className="w-3.5 h-3.5 text-[#7A7A72] hover:text-[#4A6741] shrink-0 cursor-pointer" />
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    <div className="pt-3 border-t border-[#EDEAE2] space-y-3">
                      {post.comments && post.comments.map(c => (
                        <div key={c.id} className="flex items-start gap-2.5 text-xs bg-[#F9F7F2] p-2.5 rounded-2xl">
            ...
            <div className="space-y-3">
              {classModules.length === 0 ? (
                ...
                classModules.map(mod => (
                  <div key={mod.id} className="bg-white rounded-3xl p-5 border border-[#EDEAE2] shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                      ...
                      <span className="text-xs text-[#7A7A72]">{mod.durationMinutes} min reading</span>
                    </div>
                    ...
                    {mod.attachments && (
                      <div className="flex flex-wrap gap-2 pt-2">
                        {mod.attachments.map(att => (
                          <div key={att.id} className="p-2.5 rounded-2xl bg-[#F9F7F2] border border-[#EDEAE2] flex items-center gap-2 text-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            ...
            <div className="space-y-3">
              ...
              {classAssignments.length === 0 ? (
                ...
                classAssignments.map(asg => (
                  <div key={asg.id} className="p-4 rounded-3xl bg-white border border-[#EDEAE2] shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-2">
            ...
            <div className="space-y-3">
              ...
              {classQuizzes.length === 0 ? (
                ...
                classQuizzes.map(quiz => (
                  <div key={quiz.id} className="p-4 rounded-3xl bg-white border border-[#EDEAE2] shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-2">
                    <div className="flex justify-between items-start">
      ...
      {activeTab === 'people' && (
        <div className="bg-white rounded-3xl p-6 border border-[#EDEAE2] shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-6">
          <div>
            ...
            <div className="flex items-center gap-3">
              <img src={currentClassroom.teacherAvatar} alt={currentClassroom.teacherName} className="w-10 h-10 rounded-full object-cover border-2 border-[#4A6741]" />
              <div>
                <h4 className="font-bold text-xs text-[#2D2D2A]">{currentClassroom.teacherName}</h4>
                <p className="text-[11px] text-[#7A7A72]">Faculty • {currentClassroom.subject} ({currentClassroom.roomNumber})</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {studentProfiles.map(s => (
                <div key={s.id} className="flex items-center justify-between p-2 rounded-2xl hover:bg-[#F9F7F2] transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-[#7A7A72]">Roll No. {s.rollNumber} • Section {s.section}</p>
                    </div>
                  </div>
