...
import { MessageSquare, Send, Paperclip, Search, CheckCheck, Sparkles, Phone, Video } from 'lucide-react';
...
export const MessagesView: React.FC = () => {
  ...
  const channels = [
    { id: 'chan-1', name: 'Ramesh Thapa (Math Faculty)', subtitle: 'Grade 8 Mathematics', avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150', online: true },
    { id: 'chan-2', name: 'Grade 8-A Parent-Teacher Group', subtitle: 'Official Class Updates', avatar: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=150', online: true },
    { id: 'chan-3', name: 'Saraswati Gurung (Science)', subtitle: 'Science Lab Instructor', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150', online: false }
  ];
    ...
    <div className="bg-white rounded-3xl border border-[#EDEAE2] shadow-[0_2px_10px_rgba(0,0,0,0.02)] overflow-hidden h-[80vh] flex animate-in fade-in duration-200 text-xs">
      ...
      <div className="w-80 border-r border-[#E5E1D8] flex flex-col bg-[#F0EDE5]">
        <div className="p-4 border-b border-[#E5E1D8]">
          <h2 className="font-bold text-sm text-[#2D2D2A] font-serif mb-2">School Messages & Channels</h2>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {channels.map(chan => {
            ...
            return (
              ...
              >
                <div className="relative">
                  <img src={chan.avatar} alt={chan.name} className="w-10 h-10 rounded-full object-cover shrink-0" />
                  {chan.online && (
                ...
                <div className="flex-1 truncate">
                  ...
                  <p className={`text-[11px] truncate ${isSel ? 'text-[#4A6741]' : 'text-[#7A7A72]'}`}>{chan.subtitle}</p>
                </div>
              </div>
            );
          })}
      ...
      <div className="flex-1 flex flex-col bg-white">
        ...
        <div className="p-4 border-b border-[#E5E1D8] flex items-center justify-between bg-[#F0EDE5]/40">
          <div className="flex items-center gap-3">
            <img src={currentChannel.avatar} alt={currentChannel.name} className="w-9 h-9 rounded-full object-cover" />
            <div>
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-[#F9F7F2]">
          {channelMessages.map(msg => {
            ...
            return (
              ...
              >
                <img src={msg.senderAvatar} alt={msg.senderName} className="w-7 h-7 rounded-full object-cover shrink-0 mt-1" />
                <div>
                  <div
                  ...
                  >
                    {!isMe && <p className="font-bold text-[10px] text-[#4A6741] mb-1">{msg.senderName}</p>}
                    <p>{msg.content}</p>
                  </div>
                  <span className={`text-[9px] text-[#7A7A72] block mt-1 ${isMe ? 'text-right' : ''}`}>
        ...
        {/* Message Input */}
        <form onSubmit={handleSend} className="p-3 border-t border-[#E5E1D8] flex gap-2 items-center bg-white">
