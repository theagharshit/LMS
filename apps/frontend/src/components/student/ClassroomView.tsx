...
export const ClassroomView: React.FC<ClassroomViewProps> = ({
  ...
  const subjectFilterOptions = useMemo(() => {
    const subjects = Array.from(new Set(classrooms.map((c) => c.subject))).sort();
    return [
  ...
  const getPendingTaskCount = (classroomId: string) => {
    const pendingAssignments = assignments.filter((asg) => {
      if (asg.classroomId !== classroomId) return false;
    ...
    const pendingQuizzes = quizzes.filter((q) => {
      if (q.classroomId !== classroomId || !isQuizVisibleToStudents(q) || !isQuizTakeable(q)) {
        return false;
      }
      return !quizSubmissions.find(
        (s) => s.quizId === q.id && s.studentId === currentUser.id,
      );
    });
  ...
  const landingStats = useMemo(
    () => ({
      ...
      pendingTasks: classrooms.reduce((sum, cls) => sum + getPendingTaskCount(cls.id), 0),
    }),
          ...
          {filteredClassrooms.length === 0 ? (
            ...
            filteredClassrooms.map((cls) => {
              ...
              return (
                ...
                >
                  {/* Visual header with subject banner */}
                  <div className="relative h-36 overflow-hidden">
                    {cls.bannerImage ? (
                      ...
                      <div
                        className={`absolute inset-0 bg-gradient-to-br ${cls.colorTheme || 'from-[#4A6741] to-[#6B8A5F]'}`}
                      />
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-[#2D2D2A]/85 via-[#2D2D2A]/35 to-transparent" />

                    <div className="relative z-10 h-full p-4 flex flex-col justify-between">
                      <div className="flex items-start justify-between gap-2">
                        <span className="text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-full bg-white/15 backdrop-blur-md text-[#FDEEDC] border border-white/15">
                          {cls.subject}
                        </span>
                        {pendingTasks > 0 && (
